...
int main(int argc, char** argv) {
    ...
    printf("connecting to IP %d.%d.%d.%d port %d to read %d packets of size %ld\n",
    ...
    for (i=0; i<count; i++) {
        rest=size;
        while ((n=recv(sock, buffer+size-rest, rest, 0))>0) {
            sent+=n;
            rest-=n;
        }
        if (n!=0) {
...
endconn:
    ...
    printf ("read %lld bytes in %ld.%06ld seconds from %d.%d.%d.%d, %lld MByte/s\n", 
